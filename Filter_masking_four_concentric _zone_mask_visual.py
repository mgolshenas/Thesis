...
# Create visualization zone mask
...
num_zones = len(zone_code)
max_radius_visual = np.max(distance)  # Use full distance to include corners
...
# Add labels to each zone
for i in range(num_zones):
    r = (thresholds_visual[i] + thresholds_visual[i + 1]) / 2
    label_x = cx + r / np.sqrt(2)
    label_y = cy - r / np.sqrt(2)
    ax[2].text(label_x, label_y, f'zone {i + 1}', color='black', fontsize=16, fontweight='bold')
